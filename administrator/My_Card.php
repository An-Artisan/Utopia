...
<!-- 设置一个table表格 -->
<form action="My_Card.php" method="post">
...
<?php 
  ...
  // 吧用户标识赋值给一个变量
	...
	$sql = "select * from `tb_publish` where `username` = '$username'  LIMIT ". ($curPage - 1) * $rowsPerPage . ",$rowsPerPage  ";
  // 查询用户名为当前登录用户的帖子
...
echo '<ul class="w3-pagination">';
   // 如果不是第一页的话就有上一页和首页的标记。
    if ($curPage > 1) {
        echo "<li><a class='w3-hover-sand w3-pale-blue ' href = 'My_Card.php?curPage=" . ($curPage - 1) . "'>&laquo;</a></li>";
         echo "<li><a class='w3-light-grey w3-hover-aqua' href = 'My_Card.php?curPage=1'>首页</a></li>";
            }
     for ($i = 1; $i <= $pages; $i++) {//循环显示，每个链接指定curPage属性为其指向的页数就可以了
            echo "<li><a class = 'w3-light-grey w3-hover-aqua'  href='My_Card.php?curPage=",$i,"'>$i</a></li>","&nbsp;";
         }       
        //如果不是第一页的话就有上一页和首页的标记。
        if ($curPage < $pages) {
            echo "<li><a class='w3-hover-sand w3-pale-blue' href='My_Card.php?curPage=" . ($curPage + 1) . "'>&raquo;</a></li>";
            echo "<li><a class='w3-light-grey w3-hover-aqua' href = 'My_Card.php?curPage=$pages'>末页</a></li>";
            }
